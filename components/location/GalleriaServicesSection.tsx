...
const services = [
  ...
  {
    label: "loc_service_pet_shop",
    img: "https://www.thedoghouseps.com/wp-content/uploads/2025/04/pet-shop.png",
    desc: "loc_service_pet_shop_desc",
    extra: "loc_service_pet_shop_extra",
  },
];

export default function GalleriaServicesSection() {
  const { t } = useLanguage();
            ...
            >
              ...
              >
                ...
                {"link" in svc && svc.link ? (
                  <>
                    {" "}
                    (<Link href={svc.link} style={{ color: "#965B83" }}>{t(svc.linkText as any)}</Link>)
                  </>
                ) : null}
                {"price" in svc && svc.price ? ` (${t(svc.price as any)})` : ""}
                {"extra" in svc && svc.extra ? ` ${t(svc.extra as any)}` : ""}
              </p>
            </div>
          ))}
        </div>
